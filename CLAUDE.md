...
- One centered column, typography-led: Bebas Neue Pro (display), IBM Plex Serif (19px/1.6 body), IBM Plex Sans (12px labels), Franklin Gothic URW Cond (nav, subnav, footer links). All via Typekit (`use.typekit.net/eer6kbs.css`).
- Nav (Zen 2.0): always-visible flat row — black wordmark left, 7 Franklin links at #999 right; the current section runs black + underlined (zen.js `.is-active`, matched by path prefix, then the article's `tag-*` class via each link's `data-tag`, then Notes as the unsectioned fallback). Below 900px the row becomes the burger's full-screen menu.
- No landing pages: section URLs (/radio/, /dreams/, /art-diary/, /favorites/, /public-works/, /notebook/) are routes.yaml routes rendering channel-*.hbs — ONE post per page (the latest, via post-view.hbs) with the section's full index as a bottom subnav. Post pages (post.hbs) render the same subnav, dispatched by `{{#has tag}}`. Subnav flavors: 3-col numbered list (subnav-list.hbs — Dreams/Favorites/Public Works; subnav-notebook.hbs for Notes with 8 stacked paged queries), 8-up 3:2 thumb grid (subnav-art.hbs), MR cover grid (mr-grid.hbs). zen.js marks `.is-current` (by pathname, first entry on channels) and wires the meta-line prev/next arrows from the entries.
- /about/ uses page-about.hbs: the /instructions/ post renders hidden after the bio and zen.js moves it into a `<div data-instructions></div>` HTML card placed in the About editor (unhides in place if no marker).
- Ghost gotchas learned the hard way: `{{#get}}` does NOT interpolate route-level context in filters (post-scope attributes like `{{primary_tag.slug}}` DO work — documented related-posts pattern); Ghost 6 caps `{{#get}}` at 100 posts (stack paged literal queries — see subnav-notebook.hbs); `{{#has tag}}` matches display NAMES not slugs; `kg-*` classes are emitted at runtime by the Ghost editor and must stay styled; routes.yaml + redirects.json are NOT auto-deployed (upload via Settings → Labs); MR episode numbers resolve client-side from internal `#mr-N` tags.
- Legacy product pages (custom-spite.hbs, custom-metal.hbs) carry a scoped "Zen compatibility layer" at the top of their inline <style> — reverb's tokens (--font-big, --color-hover, etc.) and layout wrappers (.content.outer, .inner-wide, .radio-content-wrapper), lifted from the archived reverb branch. They render under Zen's nav/footer. (page-static.hbs is gone — /static/ is hosted outside Ghost.)
